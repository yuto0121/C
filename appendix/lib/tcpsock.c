...
int tcp_passive_open(tcpsock_t **sock, int port) {
    ...
    const int enable = 1;
    int to_set = setsockopt(s->sd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
    TCP_DEBUG_PRINTF((to_set != 0), "setsockopt(): failed to set socket reuse addr.\n");
    TCP_ERR_HANDLER((to_set != 0), return TCP_SOCKOP_ERROR);
    to_set = setsockopt(s->sd, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(int));
    TCP_DEBUG_PRINTF((to_set != 0), "setsockopt(): failed to set socket reuse port.\n");
    TCP_ERR_HANDLER((to_set != 0), return TCP_SOCKOP_ERROR);
...
int tcp_receive_timeout(tcpsock_t *socket, void *buffer, int *buf_size, int timeout) {
    TCP_ERR_HANDLER(socket == NULL, return TCP_SOCKET_ERROR);
    TCP_ERR_HANDLER(socket->cookie != MAGIC_COOKIE, return TCP_SOCKET_ERROR);
    ...
    tv.tv_sec = timeout;
    tv.tv_usec = 0;
    int to_set = setsockopt(socket->sd, SOL_SOCKET, SO_RCVTIMEO, (const char*)&tv, sizeof tv);
    TCP_DEBUG_PRINTF((to_set != 0), "setsockopt(): failed to set socket timeout.\n");
    TCP_ERR_HANDLER((to_set != 0), return TCP_SOCKOP_ERROR);

    int requested_size = *buf_size;
    *buf_size = recv(socket->sd, buffer, *buf_size, 0);

    TCP_DEBUG_PRINTF((*buf_size < requested_size) && ((errno == EWOULDBLOCK) || (errno == EAGAIN)), "Recv() : connection timed out.\n");
    TCP_ERR_HANDLER((*buf_size < requested_size) && ((errno == EWOULDBLOCK) || (errno == EAGAIN)), return TCP_TIMED_OUT);
